(function() {
    // base path は stat/index.astro が window.READING_VIMRC_BASE に注入する
    // （現行の Liquid の site.github.url / site.baseurl 参照の置き換え）。
    // 末尾に '/' を含む（例: '/reading-vimrc/'）。
    var BASE = window.READING_VIMRC_BASE || '/';
    ...
    app.controller('UserCtrl', ['$scope', '$routeParams', '$http', '$location',
        function($scope, $routeParams, $http, $location) {
            ...
            $http({method: 'GET', url: BASE + 'json/archives.json'})
    ...
    /* d3 */
    var ARCHIVE_URL_TEMPLATE = BASE + 'archive/';
